...
import { StyleSheet, Text, View, SafeAreaView, ActivityIndicator, TextInput, Keyboard, Image, Dimensions, Platform, AsyncStorage } from 'react-native';
...
export default class Main extends React.Component {
  ...
  async componentWillMount(){
    try {
      ...
      if (lastScreen !== null & lastScreen !== undefined) {
        lastScreen = JSON.parse(lastScreen);
        if (lastScreen.lastScreen == "News") {
          this.props.navigation.navigate("Article", {url: lastScreen.url});
        }
      }
    }
    catch(err) {}
  ...
  getArticleContent = () => {
    ...
    return (
      <View style={{flex: 1}}>
        <News articles={articles} resetList={this.resetList} redirectFn={navigate}/>
        {loadingState}
      </View>
    )
  }
